    ...
    search: () => withFilterBar(
      ...
      (sections, emptyFiltered) => (
        <>
          ...
          {/* **补上第三档：还没搜。** 上面那两档少了「query 是空的」这一种，
              而这一屏**自己不带输入框**——输入框只长在 `/` 弹出的搜索弹层里
              （`SearchModal` 的 `onQuery`，见文件末尾那段），这一屏是那个弹层
              点「查看全部」之后的落点。于是直接导航进来（模块栏那个放大镜、
              旧书签、或者把弹层里的词删光）看到的是「没有匹配的任务。换个词
              试试」——**没有词，也没有能改词的地方**，一句话把人指进死胡同。
              弹层那边三档都分好了（「打字就开始找」/「没有匹配」/「这儿还没有
              任务」），这一屏照抄第一档，并说清那个框怎么叫出来。 */}
          <TaskGrid
            {...gridWiring}
            density={density}
            sections={sections}
            empty={
              tasks.length === 0
                ? EMPTY_NO_TASKS
                : query.trim() === ''
                  ? '还没搜。按 / 打开搜索框——标题、备注、标签、子任务都会搜。'
                  : '没有匹配的任务。换个词试试——标题、备注、标签、子任务都会搜。'
            }
