// 这些段是 server/src/model.ts 的**拷贝**，`lib/types.sync.test.ts` 盯着它们
// 不许飘。改一边就得改另一边。
//
// **这里原来写的理由是「两个包之间传不过来」——那句是假的**：web 侧已经有
// 十几个文件直接 `import` `../../../server/src/…`，`TaskCard.tsx` 引的还是一个
// **值**（`skipPatch`），不只是类型。真实理由是别的：跨包直接引类型会把
// server 的整棵依赖拉进 web 的类型图，而这份拷贝让 web 的构建不依赖 server
// 能不能编译过。要拆这份拷贝是可以讨论的（同步测试已经把风险压住了），
// 但别拿那句不成立的理由当依据。
//
// **别在这儿写「这几段」的数目**：上一版写死过一个数，早就飘了；改的时候
// 顺手又在同一句话里写了新的数——同一个坑当场踩第二次。CONTRIBUTING.md
// 那节「名单不写在这里」说的就是这个形状。要数就看 `types.sync.test.ts`
// 的 `NAMES`，那份自己还被一条测试对着 model.ts 的导出清点。
 ...
 * `'due'`（默认）从这一条的 `due` 往后推，周期不漂；`'done'` 从完成那一刻
 * 往后推。`count` 是「还要再重复几次」（`null` = 一直重复），仿它的
 * 「按次数结束重复」，存的是剩余次数不是总次数。`every: 'ebbinghaus'` 是
 * 艾宾浩斯记忆法，间隔不固定、走到第几步看 `step`。`monthDay` 是月重复锚在
 * 几号（1-31，`null` = 没记过），跟 `weekdays` 是一对——没有它，「每月 31 号」
 * 过一次二月会永久漂成 28 号。完整说明在 server/src/model.ts 那一份。
 */
/**
...
/**
 * 一条任务。`pinned`（置顶）跟 `order`/`priority` 同一类，是人的判断，AI 写了
 * 不算数；`parentId`（多级任务）**最多五层**（跟 `model.ts` 那份一字对齐——这儿曾经
 * 写着「只做一层，子任务不能再有子任务」，而服务端的 `checkParentLink` 一直
 * 收五层，两份说法差了整整一个功能）。完整说明
 * 在 server/src/model.ts 那一份。
 */
 ...
 * 任务都背一个永远是 null 的字段，AI 的契约（AGENTS.md 那 23 个字段）也要跟着改。
 *
