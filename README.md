...
- **两条 AI 路子**：本机命令行（支持 Claude Code、Antigravity CLI agy、OpenAI Codex、Gemini CLI、Aider 或自定义 CLI）；也可在设置里填任意 OpenAI 兼容接口
...
**一批到点，就报一件事。** 到点从来不是「一条一条来」的：早上设一批提醒、或者
一批任务本来就都排在九点，那一分钟里会一起响。以前是一条一条各弹各的——系统
通知栏里并排躺五条（每条还各起一个 PowerShell 进程）、webhook 收五条同一秒的
POST、网页上摔下三个警告框再挂一行「还有 N 条到点了」，而这五条还得一条一条
去点「稍后」。

现在**一轮扫描里到点的算一批，一批只报一条**：

- **系统通知和 webhook**：一条「5 件事到点了」，正文列出是哪几件。桌面版那条
  通知上没有「完成/推迟」两颗按钮——它说的是一批，没有哪一条可以「完成」，
  硬塞一个「完成」进去只会完成其中一条而你不知道是哪条。点通知本体打开应用。
  webhook 收到的载荷里多一个 `kind: "reminder-batch"`（**单条那一路发的是整个
  任务对象、没有 `kind`**），另带 `count` 和 `items`（每项只有 `id` 和 `title`）。
  接收端照 `kind` 分辨是哪一种，**别照着 `title` 猜**：成批的 `title` 是
  「N 件事到点了」这句话本身，不是某一条任务的名字。
- **网页横幅**：一条横幅报总数、把这几件列出来（最多五条，更多的折成「等 N 条」）。
  每条前面那颗勾选圈是**逐条完成**（跟看板上勾选同一个动作，一样有撤销提示），
  标题点得动（点了打开那条任务）；下面两颗按钮是**批量**的：「全部稍后 10 分钟」
  （旁边小箭头能换成 30 分钟 / 1 小时）和「全部知道了」。

**只有一条时还是老样子**：一条横幅、三个动作（完成 / 去做 / 稍后），一个字没改。
绝大多数提醒就是一条，为了「多条时好看」把它顺手改掉，是把最常见的那种情况
变差。

**「全部完成」这一颗没有，是有意的。**「稍后」的代价是可逆的、只关于时间
——这几条本来就在等着做，晚十分钟再来一次；「完成」不是：它会把五条没做的事
一起标成做完，而完成在这个应用里要连带改重复任务的下一次、连带完成子任务，
一次误点就是一串假数据。真要逐条结案，去列表上勾，那里勾错一条只错一条。

**「同一批」的粒度在两个壳里不一样，各自用的是它能拿到的最准的那个**：桌面和
网页按**服务端那一轮扫描**（30 秒一轮），手机按**同一时刻**——手机上的通知是
提前排好的本地闹钟，它不知道「这一轮」是什么，但它知道两条提醒是不是同一刻。
（差一分钟的两条**不合并**：那是你自己分开设的两条，并成一条等于把其中一条的
时刻静默吞掉。）两边报出来的文案形状一样（「N 件事到点了」+ 标题列表），
同一次到点在哪台设备上说的都是同一句话。
...
**这一段说的是「只有一条到点」时的那条横幅**（绝大多数提醒就是这一种，一个字
没改）。同时到点好几条时是另一条合成的横幅、动作也是批量的，见上面
「同时到点的几条，合成一条」。
...
默认这条路是起一个命令行子进程（支持 **Claude Code**、**Google Antigravity CLI (`agy`)**、**OpenAI Codex (`codex`)**、**Gemini CLI (`gemini`)**、**Aider (`aider`)** 以及**自定义 CLI**）——它自己读 `AGENTS.md`、自己读写 `data/`，
能反复读几十个文件、自己纠正自己，本事最大。可在**设置 → AI 拆解 → 命令行工具**中自由切换、指定可执行文件路径，或者填入自定义命令与参数模板（支持 `{prompt}` 占位符）。

如果本机没有装相关 Agent CLI，可在**设置 → AI 拆解 → 怎么叫 AI**切到「调接口」，填三格就行：接口地址、模型、密钥。
...
那几个名字在设置里是可以点的标签，点一下地址和模型一起填好；「模型」那格也是可点的
下拉，地址落在目录里哪一家的端点上，就出**它家那份**模型清单（显示的是好读的名字，
点一下填进框里的还是原样发接口的那个 id）。地址框照样能自己写，**这不是白名单**——
手填的地址反查不到候选，那格就退化成纯手填，一个字都不拦。地址填到 `/v1` 或者光一个
域名也认，服务会补齐。

预置那行是直接从上游同步下来的 provider 目录里派出来的（`web/src/lib/aiPresets.ts`），
目录里加一家、界面就自动多一家，不用回来改代码。派的时候按目录自己的标记来：只有
OpenAI 协议的那几家入（服务端只讲 Chat Completions，Anthropic / Gemini 原生端点填进去
就是 400）；同一家的地域 / 计费变体只留一个；「一个端点就是一个独立产品」的才各自成
标签，本机那几个（Ollama、LM Studio、llama.cpp、koboldcpp、LiteLLM）就是这条。
...
一条任务卡在那儿不动，多半不是不想做，是**它太大了**——「装修」三个字摆在卡片上，
看着就不知道从哪儿下手。收件箱里的**文本**有 AI 帮你拆，可一旦它变成任务、
落到看板上，原来就只能自己一条条手打子任务。

现在卡片 ⋯ 里多了一项「让 AI 拆细」。它跟上面那个「拆解」是两件事：拆解读的是
收件箱里的原始文本、产出的是新任务；拆细看的是**一条已经在看板上的任务**、
产出的是它底下的子任务。

- **它只发这一条。** 加上它已有的直接子任务（孙子不发），别的任务一概不看，
  也不许对它们提建议或观察。这是四件 AI 活里唯一**必须带范围**的一件——
  「拆细」不给宾语就是一句没头没尾的指令。所以服务端那层 `taskId` 是必填的，
  对不上直接 400，**不会退回去拆全量**（退回去等于替你把一次点错变成一次全额消费）。
- **它只提子任务，别的字段一个都不动。** 这条已经挂在看板上了，标题、期限、
  提醒都是你亲手填的，AI 这时候捎带一句「顺便帮你把截止时间改了」是越界。
  能提的字段名单正本还是 `PROPOSABLE`，这份约定只用了其中一个。
- **提建议的路子跟回顾一模一样**：落在卡片上，群青的一小块，你点「接受」才真的
  生效。**接受是整份换掉**（不合并、不留旧版），所以约定里写明 AI 必须把已有的
  那几条连同你勾过的 ✓ 原样带上——漏掉哪条，你点一下接受那几步就当场消失了。
- **已经够具体的就说没有。** 「给张老师打个电话」这种它回一句没有建议，不硬凑。
- **习惯这一项压根不出现**（`habit: true` 的任务）。习惯在这套模型里是「每天
  重复的那件事」，没有「第一步」可拆——摆出来就是一次注定没有产出的调用，
  而回执那句「这条可能已经够具体了」对它是错的说法。
- **已经了结的也不出现**：做完、搁置、放弃的任务，⋯ 里没有这一项。搁置的意思是
  「暂时不想看见它」，这时候递上来一份拆解方案，正好是那句话的反面。
- 跟拆解、回顾、总览**共用同一把单飞锁**，四件不会同时跑。
- **也不自动跑**，跟回顾同一个理由：要花一两分钟和一次额度，节奏你定。

命令行那条路一样能叫：AI 会话里敲 `/breakdown 装修`（得把是哪一条说出来，
不然它不知道拆哪条），或者直接说「把这条任务拆细」。
...
同一屏上「让 AI 回顾一遍」旁边还有一颗「AI 总览」。它跟回顾是两种东西：**回顾逐条
挑毛病、产出挂在卡片上等你接受；总览只读一遍，把现在整个看板什么样用一段大白话
讲给你听**——待办/进行中/搁置各多少张、哪些过期了、哪些在「进行中」躺了太久、
收件箱还压着几条、有没有重复记了好几遍的同类事。约定在 `workflows/board.md`。

- **它不改任何数据**：不拆任务、不提建议、不写 outbox，跑完没有任何卡片发生变化。
  它的产物就是跑完后横幅里那段汇报本身——所以那条横幅**不会自动消失**（拆解/回顾
  的成功提示几秒就自己收走，因为结果在别处；总览的提示就是结果），读完手动关。
- **也不自动跑**，跟回顾同一个理由：每次要花一两分钟和一次额度，节奏你定。命令行
  一样能叫：AI 会话里敲 `/board`，或者直接说「汇总当前看板状态」。
- 跟拆解、回顾、拆细共用同一把单飞锁——四件不会同时跑，点的时候上一个还没完，
  这一颗会被挡回来。
...
**最上面还有一节「这一周的进出」**——完成 N 条 · 新进来 M 条 · 净 ±K 条，滚动
最近 7 天。这一屏别的每一行说的都是**欠着的**（还压着几条、过期几条、搁了多久），
全是负债；只有这一行说**还掉了多少**。一个只报债、从不报还债的系统，用久了会让人
觉得自己一直在落后——而事实未必如此。两个口径坑直接写在界面上：已经删掉的不在
里面（软删除进了回收站，这一屏数不到），所以那个「净」**偏向乐观**；它算的是
**当前数据**的投影、不是一份事件流水（一条做完又打开的任务，那个完成时刻就没了）。
用滚动 7 天不用自然周，是因为「周一早上打开」只有一小时的样本。**两件都没发生
的时候整节不出现**——一个「完成 0 条 · 新进来 0 条」的横幅是纯噪音。
...
- **N 条改过 2 次以上期，一直没动**——就是「一拖再拖」。这一行底下也直接列着
  它们，每条写着推了 N 次。**它要的不是再推一次**：「加到今天」那个动作你已经在
  推荐面板里做过很多遍了，缺的是「还要不要它」——要就拆小到能动手（卡片 ⋯ 里
  有「让 AI 拆细」），不要就搁置或者放弃掉。门槛跟推荐面板那一组是同一个常量，
  但**候选池不一样是有意的**：一条天天被推的任务很可能就赖在「今天」里，而它
  恰恰是这一节最该问的那一条。
- **N 条在等别人、超过 3 天没动静**——「在等谁」填了、却一直没动的那些。
  该催了吗
- **N 条搁了超过 30 天**——「将来也许」那一堆。还要吗

**数为 0 的那一行整行不渲染**：一份全是「0 条」的清单读起来像有六件事要办，
而实际一件都没有。这几行全空、上面那节进出也是 0 时，这一屏只剩底部那颗 AI 按钮。
...
本月打了几天，底下是这个月的打卡表（一天一格，**按周排成七列**、顶上带一行
星期标签）。点习惯名字跳回那条待打卡的
任务。
...
- **本月的分母是「这个习惯这个月能打卡的天数」**，不是整月天数，也不是
  ...
  **分母算出来是 0 时「本月」那一栏整个不渲染**——那是「今天刚建了一个每周
  几次的习惯、而今天不是打卡日」：分子分母一起是 0，屏幕上就是「本月 0 / 0 天」，
  一个读起来像坏掉的分数。上面那条防的是「欠账感」，这条防的是「不成句」；
  同一套理由的第三次收窄。
...
**开着应用出去半天，回来时提醒横幅不会把看板顶没。** 同时到点的那一批合成
一条横幅（见上面「同时到点的几条，合成一条」），一条里最多列五件、更多的折成
「等 N 条」——看板还在屏幕上，而你想看的恰恰是看板。

那条横幅上的「全部知道了」只把横幅摘掉，**不动任务也不动提醒**——跟每条那颗
× 一样。它在这一档是常驻的（不再是「摆不下才出现」）：它清的就是眼前这一摞，
误点的代价比它省的那几下小。
